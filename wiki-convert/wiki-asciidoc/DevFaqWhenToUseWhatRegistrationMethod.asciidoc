...
As described in link:DevFaqModulesGeneral[DevFaqModulesGeneral], there are several different declarative registration mechanisms:

* Use the `@ServiceProvider` annotation (or one of the link:DevFaqWaysToRegisterInDefaultLookup[other registration mechanisms]) to register objects in the link:DevFaqLookupDefault[default Lookup]
* Add files to folders in the link:DevFaqSystemFilesystem[system filesystem] 
...
If you are implementing some API from another module, that module should tell you what to do.  If it tells you something should be in the link:DevFaqLookupDefault[default lookup], that means to use [link:http://bits.netbeans.org/dev/javadoc/org-openide-util-lookup/org/openide/util/lookup/ServiceProvider.html[http://bits.netbeans.org/dev/javadoc/org-openide-util-lookup/org/openide/util/lookup/ServiceProvider.html] `@ServiceProvider`] (see caveats in link:DevFaqWaysToRegisterInDefaultLookup[DevFaqWaysToRegisterInDefaultLookup]).

==== Deciding On A Registration Mechanism For Clients Of Your Module

If you are defining an link:DevFaqApiSpi[SPI] in your module, and other modules will implement it and provide their own classes, provide a declarative (plain text, no code) way to register them.

Define interfaces or abstract classes, and document where subclasses can be registered (typically the link:DevFaqLookupDefault[default Lookup] or some folder in the link:DevFaqSystemFilesystem[system filesystem]).

Starting with NetBeans 6.7, you can link:DeclarativeRegistrationUsingAnnotations[provide annotations] which other modules can use to register their objects - so registration is declarative, but it is visible in the Java source file.

If you can possibly avoid it, don't require your module (or modules that implement your link:DevFaqApiSpi[SPI]) to run code on startup to link:DevFaqModulesDeclarativeVsProgrammatic[programmatically register] their functionality.  That slows startup time and does not scale.
