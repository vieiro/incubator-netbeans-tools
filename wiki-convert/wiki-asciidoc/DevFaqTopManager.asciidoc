...
The generation of NB it was written for is from before Lookup (see link:DevFaqLookup[DevFaqLookup]) was in use. `TopManager` was a class with a bunch of static methods for getting service objects.
It is now gone.
