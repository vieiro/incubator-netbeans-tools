...
link:FaqNetBeansAndOOME[FaqNetBeansAndOOME] describes what options can be used for this.
...
`NbTestCase.assertGC` is all you need to know. See also link:FitnessMemoryLeaks[FitnessMemoryLeaks].

link:FitnessViaTimersCounter[Timers/counters] module can be used to register objects of interest in the code, then inspect them during IDE run via Runtime Watches window.
