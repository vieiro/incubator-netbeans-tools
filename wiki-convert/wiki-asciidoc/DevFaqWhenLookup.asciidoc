...
Some other module can then contribute (most likely via a link:DevFaqModulesLayerFile[layer file] and Lookups.forPath()) a Displayable for that object.  This is a rather extreme form of extensibility and can be hard to debug, but if you need it, Lookup can be used for that.
...
See also: link:DevFaqWhenToUseWhatRegistrationMethod[When To Use Various Declarative Registration Mechanisms]
