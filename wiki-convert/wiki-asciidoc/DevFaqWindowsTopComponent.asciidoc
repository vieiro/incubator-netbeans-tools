...
TopComponents live inside link:DevFaqWindowsMode[Mode]s - docking containers.
...
* *visible/invisible* - When a TopComponent is _visible_, it is the selected tab in its link:DevFaqWindowsMode[Mode]
* *active/inactive* - When a TopComponent is _active_, it or a child component of it has keyboard focus, and it is currently the global link:DevFaqTrackGlobalSelection[selection context]

Each `TopComponent` has a `link:DevFaqLookup[Lookup]` and one or more `link:DevFaqWhatIsANode[activated Node]`s.  These define the link:DevFaqTrackGlobalSelection[selection context] for the window system, which determines what actions (menu items, toolbar buttons, etc.) are enabled, and in some cases, what they will do or operate on if invoked.
...
For information on how and when these are reconstructed on startup, see the link:DevFaqWindowsInternals[gory details of the window system on startup].
