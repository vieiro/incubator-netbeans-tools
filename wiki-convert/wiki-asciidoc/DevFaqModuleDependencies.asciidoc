...
Modules can also load classes from libraries - JAR files that are packaged with the module (see link:DevFaqHowPackageLibraries[DevFaqHowPackageLibraries]).
...
you probably want to read link:DevFaqWrapperModules[DevFaqWrapperModules] and also link:DevFaqWhenUseWrapperModule[DevFaqWhenUseWrapperModule].
