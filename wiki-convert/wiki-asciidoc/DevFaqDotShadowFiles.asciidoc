...
`*.shadow` files are mainly used in the link:DevFaqSystemFilesystem[system filesystem]
...
is created in the link:DevFaqModulesLayerFile[module's layer file], in the `Actions/` folder.
