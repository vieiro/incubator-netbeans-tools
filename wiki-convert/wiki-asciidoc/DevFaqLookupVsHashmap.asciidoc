...
1. it is impossible to enforce link:DevFaqModuleDependencies[dependencies].  With `link:DevFaqLookup[Lookup]`, a module's code cannot request an object of type X unless it can load/access the `X.class`. The module trying to look up an X will not be able to see `X.class` unless it link:DevFaqModuleDependencies[declares a dependency] on the module that defines X _and_ the module which defines X says that it allows access to the Java package X lives in. (A `Map<Class<T>,T>` would do the same job as `Lookup`.)
