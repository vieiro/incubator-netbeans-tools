...
There is a huge link:NetBeansDeveloperFAQ[Frequently Asked Questions for Module Developers] list.  It is worth bookmarking.
...
You can see useful documentation about Mercurial and also about its specifics for NetBeans repository in link:HgMigrationDocs[HgMigrationDocs] wiki topic. If you are already familiar with Mercurial you cat go directly to link:HgHowTos[HgHowTos] topic.
...
Here's how to link:HgHowTos#Doing_your_first_build[build it].

The build of NetBeans will be created in `nbbuild/netbeans`.

=== How To Find Useful APIs

See the link:DefFaqMapApisToTasks[tasks to APIs faq] for a list of common tasks and what APIs you will want to use to accomplish those tasks.
...
First get platform sources from link:http://www.netbeans.info/downloads/dev.php[download page] or use Hg client as described in link:HgHowTos[HgHowTos].
