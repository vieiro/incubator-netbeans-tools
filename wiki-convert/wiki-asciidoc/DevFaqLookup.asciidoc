...
There is the link:DevFaqLookupDefault[global lookup] which is used to find objects (often, but not always, singletons) that are registered throughout the system.  Also, many types of objects have a method getLookup() that enables other code to get things specific to that object.  In particular, `link:DevFaqWhatIsANode[Node]`s and `Project` objects have a `Lookup`.
...
This brings the link:apidesign:MVC[MVC] design pattern into link:apidesign:DCI[modular loosely coupled] world.
...
The fact is that it is the Image module that makes it possible to open `.png` files in link:NetBeans[NetBeans].  But the POV-Ray tutorial does not need to know or care that the Image module exists, or what it does - it simply says "open this".

The common pattern you'll see for Lookup usage is one where there are three components:

* Module A is pure link:API[API] - it provides some interfaces
...
Some other module entirely actually registers an implementation of this interface in the link:DevFaqLookupDefault[default Lookup].  link:http://bits.netbeans.org/dev/javadoc/org-openide-awt/org/openide/awt/StatusDisplayer.html[StatusDisplayer] is a good example of this pattern.
