...
|`java.io.File` and `java.io.File[]` |_See the link:DevFaqNodePropertyForFiles[separate FAQ entry for File properties]_ 
...
|`java.lang.Object` (yes, you can have a property of Object and there is an editor for it - the user can select from all objects of a type in the link:DevFaqDefaultLookup[default Lookup] or a specific lookup [see below] using a combo box) |`superClass` |`java.lang.Class` |The superclass, passed to `Lookup.getDefault().lookupAll()` to find all possible values 
