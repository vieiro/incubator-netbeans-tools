...
`FileObject`s (link:DevFaqFileObject[DevFaqFileObject]) in a folder have no defined order by default.
You can use `FileUtil.getOrder` to sort them.
(`DataObject`s (link:DevFaqDataObject[DevFaqDataObject]) in a folder are always sorted this way.)
The order is determined by numeric `position` attributes.
For all details, see: link:FolderOrdering103187[FolderOrdering103187]
