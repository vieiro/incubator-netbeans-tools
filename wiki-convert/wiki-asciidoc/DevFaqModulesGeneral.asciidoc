...
declarative (link:DevFaqModulesDeclarativeVsProgrammatic[DevFaqModulesDeclarativeVsProgrammatic]); these mechanisms are preferred.

If you are writing a module that has an API you want other modules to plug in to, you probably want to read link:DevFaqWhenToUseWhatRegistrationMethod[DevFaqWhenToUseWhatRegistrationMethod].
...
The system filesystem (see link:DevFaqSystemFilesystem[DevFaqSystemFilesystem]) allows for more detailed configuration when registering objects.
It is a virtual filesystem composed of XML fragments (see link:DevFaqModulesLayerFile[DevFaqModulesLayerFile])
...
For more details on registering objects, defining an order in which they should appear, etc., see link:DevFaqModulesLayerFile[DevFaqModulesLayerFile].
...
see: link:DevFaqModulesDeclarativeVsProgrammatic[DevFaqModulesDeclarativeVsProgrammatic]
