...
At least in theory, a link:DevFaqWindowsTopComponent[TopComponent] can exist in more than one link:DevFaqWindowsMode[Mode], so there is a one-to-many relationship.  The pre-NetBeans 3.6 windowing system had a concept of "workspaces", and a Mode could be opened on multiple workspaces.  So instead of putting TopComponents (as represented by the .settings files) into the folders directly, you put those link:DevFaqDotSettingsFiles[settings files] into the `Windows2Local/Components` folder.  And you put a Window System Top Component Reference - or .wstcrf - lovely to pronounce - in the mode folder.  It's like a symbolic link, pointing to the link:DevFaqDotSettingsFiles[.settings] file in the `Windows2Local/Components` folder via its ID.  That way, one `TopComponent` could be linked to by several `Mode`s.
