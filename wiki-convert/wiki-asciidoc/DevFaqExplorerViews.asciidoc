...
An explorer view is a GUI component which can display a link:DevFaqWhatIsANode[Node ] and (optionally) its child nodes.  While Nodes are, by definition, a tree structure, explorer views are much more than just JTrees.  Here is a list of the components available:
...
A common usage is to get a Node for some folder on disk or in the link:DevFaqSystemFilesystem[configuration filesystem], optionally create a `FilterNode` to filter out some child nodes of it or its children, and display that.
