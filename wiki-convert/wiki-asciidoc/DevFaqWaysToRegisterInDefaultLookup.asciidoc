...
There are two ways to put something in the link:DevFaqLookupDefault[default Lookup]:
...
|Put an `.instance` file in your module layer in `Services/Hidden` |Rarely - useful if you are making services available dynamically on the fly, for example when link:DevFaqDynamicSystemFilesystem[dynamically changing layer contents], or when link:DevFaqFileAttributes[specialized registration metadata] or factory methods are needed |Declare a link:DevFaqInstanceDataObject[.instance file], a link:DevFaqDotSettingsFiles[.settings file] or similar, or a link:DevFaqDotShadowFiles[.shadow file] in your module's link:DevFaqModulesLayerFile[layer.xml file] 
|===
