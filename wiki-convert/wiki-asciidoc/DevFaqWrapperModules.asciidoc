...
Since you can't modify the NetBeans classpath directly (link:DevFaqNetBeansClasspath[DevFaqNetBeansClasspath]),
...
There are other options for packaging libraries described in link:DevFaqWhenUseWrapperModule[DevFaqWhenUseWrapperModule].

If the above was confusing, read link:DevFaqModuleDependencies[DevFaqModuleDependencies].
