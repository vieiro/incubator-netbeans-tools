...
Or imagine you have the situation described in link:DevFaqRequestProcessor[the FAQ about RequestProcessor.getDefault()] - a node for a file needs to read the file after it is created to mark itself if the file has errors.  `RequestProcessor.Task` makes this sort of thing easy.
