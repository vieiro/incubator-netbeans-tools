...
NetBeans is a Java application.  It has a very small core runtime which knows how to find the modules that make up the application (the launcher passes a list of directories - these are commonly called link:DevFaqWhatIsACluster[clusters] - which contain module JAR files and some XML metadata about them).
...
One distinction about NetBeans modules, as opposed to just working with JAR files on your classpath is that the NetBeans runtime link:DevFaqModuleDependencies[enforces dependency management] between modules - to call code in another module from yours, your module must _declare a dependency_ on the other module.
