...
You can actually use older platform as long as you configure it to use newer harness (either via `Tools -> NetBeans Platforms` in IDE or by specifying link:DevFaqNbPlatformAndHarnessMixAndMatch[harness.dir]) and you develop in new enough IDE.

If you cannot even use new harness and/or IDE, you have to use _suite chaining_, build your own platform and depend on it. See `harness/README` file for details. See also link:HowToReuseModules[HowToReuseModules].
...
* link:DevFaqGeneralWhereIsPlatformHowToBuild[Where is the Platform and how can I build it?] for more details on how to add a custom platform for your application so the project is stand alone.
